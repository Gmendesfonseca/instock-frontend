...
import { IUser } from '@/header-app/interfaces/User';
import { decode } from '@/header-app/utils/crypto';
import { useHTMLHead } from '@/header-app/hooks/useHTMLHead';
import { useRender } from '@/header-app/hooks/useRender';
import {
  INotificationProps,
  INotificationWrapper,
} from '@/header-app/interfaces/Notification';
import {
  getNotifications,
  NotificationParamsType,
} from '@/header-app/services/notifier/notifications';

import { NotificationUseCase } from '@/header-app/components/Notifications/NotificationUseCase';
...
interface NotificationContextProps {
  ...
  updateNotifications(
    ...
    onResponse?: (data: INotificationWrapper) => void
  ): void;
...
const manager = new Manager(links.notification);
const socket = manager.socket('/');

export const NotificationSocketContext =
  createContext<NotificationContextProps>({} as any);

const NotificationSocketProvider: React.FC<
  React.PropsWithChildren<NotificationSocketProviderProps>
> = ({ api, user, children }) => {
  ...
  useEffect(() => {
    const { openDropdownKey, closeDropdownKey } =
