...
const PermissionsProvider: React.FC<React.PropsWithChildren<unknown>> = ({
  ...
  const companyId =
    user.type === 'PERSON' && companySelected
      ? companySelected
      : user.profile_id;

  const getAllPermissionsList = async (companyId: string) => {
    const response = await api.get(links.api + '/user/permissions', {
      headers: {
        companyId,
      },
    });
  ...
  const checkPermission = (slugs: string[]) => {
    const hasPermission = slugs.every((slug) =>
    ...
    if (!!company || user.type === 'COMPANY') {
      getAllPermissionsList(company?.id ?? user.profile_id)
        .then((response) => {
          ...
          if (user.type === 'PERSON') {
            const hasVacationPermission = response.some(
              (permission: IPermission) =>
                permission.slug === 'managers_vacations_list'
            );
